...
type ClientManager struct {
	mu              sync.RWMutex
	reveniumClients map[string]*ReveniumGroq
}

// NewClientManager creates a new client manager
func NewClientManager() *ClientManager {
	return &ClientManager{
		reveniumClients: make(map[string]*ReveniumGroq),
	}
}

// GetReveniumClient retrieves or creates a Revenium client for the given key
func (cm *ClientManager) GetReveniumClient(key string, cfg *Config) (*ReveniumGroq, error) {
	cm.mu.RLock()
	...
	// Create new client
	client, err := NewReveniumGroq(cfg)
	if err != nil {
		return nil, err
	}
...
func (cm *ClientManager) CloseAll() error {
	...
	// Clear caches
	cm.reveniumClients = make(map[string]*ReveniumGroq)
